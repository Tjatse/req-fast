...
var AGENT_DEFAULT = 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/27.0.1453.110 Safari/537.36';
...
function RequestStream(options) {
  ...
  if (typeof options == 'string') {
    options = { uri:options };
  } else if (typeof options != 'object' || !options.uri) {
...
RequestStream.prototype.request = function () {
  ...
  // send data.
  this.options.data && req.write(this.options.data);
...
RequestStream.prototype.genOptions = function (options) {
  ...
  // requests from browser, LOL.
  if (!headers['user-agent']) {
    headers['user-agent'] = AGENT_DEFAULT;
  }
  if (options.data) {
    headers['content-length'] = Buffer.byteLength(options.data, 'utf-8');
  }
