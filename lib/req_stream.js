...
RequestStream.prototype.request = function () {
  ...
  var req = this.client.request(this.options, function (res) {
    ...
    if(status < 200 || status > 207){
      // error status
      var errMsg = status + ' ' + (http.STATUS_CODES[status] || 'status code.');
      var statusError = new Error(errMsg);
      statusError.statusCode = status;
      return this.emit('error', statusError);
    }
...
RequestStream.prototype.genOptions = function (options) {
  ...
  if (options.data) {
    if(!!~['POST', 'PUT', 'PATCH'].indexOf(options.method)){
      ...
      headers['content-type'] = (headers['content-type'] || '') + ';' + dataUtil[options.dataType].contentType;
